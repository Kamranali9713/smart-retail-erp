...
const REF_TYPES = ["SALE", "SALE_RETURN", "PURCHASE", "PURCHASE_RETURN", "EXPENSE", "INCOME", "CUSTOMER_PAYMENT", "VENDOR_PAYMENT", "MANUAL"];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-secondary"><tr><th className="p-3 text-left">Date</th><th className="p-3 text-left">Account</th><th className="p-3 text-left">Journal</th><th className="p-3 text-left">Reference</th><th className="p-3 text-left">Description</th><th className="p-3 text-right">Debit</th><th className="p-3 text-right">Credit</th><th className="p-3 text-right">Balance</th></tr></thead>
          <tbody>{data.rows.map((row) => <tr key={row.id} className="border-t border-border"><td className="p-3 whitespace-nowrap">{new Date(row.createdAt).toLocaleString()}</td><td className="p-3">{row.account?.name}<div className="text-xs text-muted-foreground">{row.account?.type}</div></td><td className="p-3"><span className="font-medium">{row.journalEntry?.entryNo || "—"}</span></td><td className="p-3"><span>{row.refType || "MANUAL"}</span>{row.refId && <div className="text-xs text-muted-foreground truncate max-w-32">{row.refId}</div>}</td><td className="p-3 min-w-48">{row.description || "—"}</td><td className="p-3 text-right">{row.debit ? formatCurrency(row.debit) : "—"}</td><td className="p-3 text-right">{row.credit ? formatCurrency(row.credit) : "—"}</td><td className="p-3 text-right font-medium">{formatCurrency(row.balance)}</td></tr>)}</tbody>
          <tfoot className="bg-secondary font-semibold"><tr><td className="p-3" colSpan="5">Period Totals</td><td className="p-3 text-right">{formatCurrency(data.totals?.debit || 0)}</td><td className="p-3 text-right">{formatCurrency(data.totals?.credit || 0)}</td><td className="p-3 text-right">{selectedAccount ? formatCurrency(data.closingBalance || 0) : "—"}</td></tr></tfoot>
        </table></div>
